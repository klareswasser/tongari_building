...
AREA_SOUTH = round(CENTER_LAT - _dlat, 3)   # ≈35.416
AREA_NORTH = round(CENTER_LAT + _dlat, 3)   # ≈35.955
AREA_WEST  = round(CENTER_LNG - _dlng, 3)   # ≈139.421
AREA_EAST  = round(CENTER_LNG + _dlng, 3)   # ≈140.085
...
DB_FILE       = OUT_DIR / "circle30_triangle_buildings.geojson"
HTML_FILE     = OUT_DIR / "circle30_triangles_map.html"
PROGRESS_FILE = OUT_DIR / "circle30_progress.json"
...
def process_strips():
    ...
    print(f"皇居中心 半径{RADIUS_KM}km圏を南北 {N_STRIPS} ストリップに分割 "
...
def deploy_to_web(features, total_buildings):
    """web/data にデータをコピーし、index.html を更新"""
    import shutil
    WEB_DATA.mkdir(parents=True, exist_ok=True)

    # GeoJSON をコピー
    shutil.copy2(DB_FILE, WEB_DATA / "circle30_triangle_buildings.geojson")
...
<title>皇居中心 半径30km 三角形建物マップ</title>
...
// 皇居中心から半径30kmの円
const imperialPalace = [{CENTER_LAT}, {CENTER_LNG}];
L.circle(imperialPalace, {{
  radius: 30000,
  color: '#999', weight: 1.2, dashArray: '5 4',
  fillColor: '#999', fillOpacity: 0.03, interactive: false,
}}).addTo(map);
L.marker(imperialPalace, {{
  icon: L.divIcon({{ className: 'scale-circle-label',
    html: '皇居中心 半径 30 km', iconSize: [120, 16], iconAnchor: [60, 8] }}),
  interactive: false,
}}).addTo(map);

fetch('data/circle30_triangle_buildings.geojson')
  .then(r => r.json())
  .then(data => {{
    L.geoJSON(data, {{
  ...
  .catch(e => {{
    ...
    print(f"Web 更新: {WEB_DIR / 'index.html'}")


# ── メイン ─────────────────────────────────────────
if __name__ == "__main__":
    print("=" * 60)
    print(f"皇居中心 半径{RADIUS_KM}km 三角形建物 検出")
    print("=" * 60)
